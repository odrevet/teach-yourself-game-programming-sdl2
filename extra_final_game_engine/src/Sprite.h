...
namespace TYGP
{
  ...
  public:
    ...
    Sprite(Image *pImage);
    Sprite(Image *pImage, SDL_Rect &rcBounds,
           BOUNDSACTION baBoundsAction = BA_STOP);
    Sprite(Image *pImage, SDL_Point ptPosition, SDL_Point ptVelocity, int iZOrder,
    ...
    int GetWidth()
    {
      return (m_pImage->GetWidth() / m_iNumFrames);
    };
  ...
  {
    // int iXShrink = (this->GetWidth() / 12;
    // int iYShrink = (this->GetHeight()) / 12;
  ...
  inline void Sprite::SetPosition(int x, int y)
  {
    m_rcPosition.x = x;
    m_rcPosition.y = y;
    m_rcPosition.w = this->GetWidth();
    m_rcPosition.h = this->GetHeight();
    CalcCollisionRect();
  }
